...
function FormCardFieldsComponent( {formSubmission}: any ) {
  ...
  useEffect(() => {
    setFields(formSubmission.fields);
    setNewFieldTypeId(AppContextState.fieldTypes[0].id);
  }, [formSubmission]);

  const deleteField = (field: any) => {
    http.delete(`/form_submission/${formSubmission.id}/field/${field.id}`).then((res) => {
      setFields(fields.filter((fieldRecord: any) => {
  ...
  const saveNewField = () => {
    http.post(`/form_submission/${formSubmission.id}/field`, {
      name: newFieldName,
...
export {FormCardFieldsComponent};
