...
function EmailCardFieldsComponent( {emailSubmission}: any ) {
  ...
  const deleteField = (field: any) => {
    http.delete(`/email_submission/${emailSubmission.id}/field/${field.id}`).then((res) => {
  ...
  const saveNewField = () => {
    http.post(`/email_submission/${emailSubmission.id}/field`, {
      name: newFieldName,
...
export {EmailCardFieldsComponent};
